...
(define-syntax-rule (incompatible-hashes-err dd1 dd2)
...
(define/dd-match (ddict-keys-subset? dd1 dd2)
  [(iddict elems1 _ _)
   ...
   (cond [(hash-equal? elems1)
          (unless (hash-equal? elems2) (incompatible-hashes-err dd1 dd2))]
         [(hash-eqv? elems1)
          (unless (hash-eqv? elems2) (incompatible-hashes-err dd1 dd2))]
         [else ;; hasheq
          (unless (hash-eq? elems2) (incompatible-hashes-err dd1 dd2))])
...
(struct immutable-ddict (elems del seq)
  ...
  [(define dict-ref ddict-ref)
   ...
   (define dict-has-key? ddict-has-key?)
   (define dict-set* ddict-set*)
   (define dict-update ddict-update)
   (define dict-map ddict-map)
   (define dict-for-each ddict-for-each)
   (define dict-empty ddict-empty?)
   (define dict-count ddict-count)
   (define dict-copy ddict-copy)
   (define dict-clear ddict-clear)
   (define dict-keys ddict-keys)
   (define dict-values ddict-values)
   (define dict->list ddict->list)
   (define in-dict in-ddict)
   (define in-dict-keys in-ddict-keys)
   (define in-dict-values in-ddict-values)])
